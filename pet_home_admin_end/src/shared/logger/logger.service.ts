...
import { LoggerService as NextLoggerService } from '@nestjs/common';
import { clc, yellow } from '@nestjs/common/utils/cli-colors.util';
import { join } from 'path';
import { isDev } from 'src/config/env';
import { getAppRootPath } from 'src/utils/app_route_path.util';
...
@Injectable()
// 实现接口
export class LoggerService implements NextLoggerService {
  private static lastTimestampAt?: number; // 上次的时间节点
  /**
   * 日志文件存放文件夹路径
   */
  ...
  protected printMessages(
    ...
    context = '',
    logLevel: WinstonLogLevel = 'info',
    writeStreamType?: 'stdout' | 'stderr', // 标准错误 | 标准输出
  ) {
  ...
  private getColorByLogLevel(level: WinstonLogLevel): (text: string) => string {
    switch (level) {
      case 'debug':
        return clc.magentaBright; // 明亮品红色
      case 'warn':
        return clc.yellow;
      case 'error':
        return clc.red;
      case 'verbose':
        return clc.cyanBright; // 明亮青色
      default:
        return clc.green;
    }
  }
